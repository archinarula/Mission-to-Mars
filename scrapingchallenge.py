...
def hemisphere_image(browser):
    ...
    html = browser.html
    search_page_soup = soup(html, 'html.parser')

    # Add try/except for error handling
    try:
        # Find the number of pictures to scan
        pics_count = len(search_page_soup.select("div.item"))

        # for loop over the link of each sample picture
        for x in range(pics_count):
            # Create an empty dict to hold the search results
            results = {}
            # Find link to picture and open it
            link_image = search_page_soup.select("div.description a")[x].get('href')
            browser.visit(f'https://astrogeology.usgs.gov{link_image}')
        
            # Parse the new html page with soup
            html = browser.html
            result_details_soup = soup(html, 'html.parser')
            # Get the full image link
            img_url = result_details_soup.select_one("div.downloads ul li a").get('href')
            # Get the full image title
            img_title = result_details_soup.select_one("h2.title").get_text()
